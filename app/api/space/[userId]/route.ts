...
export async function GET(req: NextRequest, { params }: { params: Promise<{ userId: string }> }) {
  ...
  const space = await dbGet<Space>('SELECT spaces.*, users.name as owner_name, users.display_name, users.bio FROM spaces JOIN users ON spaces.user_id = users.id WHERE spaces.user_id = ?', [userId]);
