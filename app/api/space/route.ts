...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const space = await dbGet<Space>('SELECT spaces.*, users.name as owner_name FROM spaces JOIN users ON spaces.user_id = users.id WHERE spaces.user_id = ?', [session.userId]);
